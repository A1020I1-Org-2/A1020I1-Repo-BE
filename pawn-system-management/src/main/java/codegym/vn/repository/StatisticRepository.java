...
public interface StatisticRepository extends JpaRepository<Contract,String> {
    @Query(value = "select *" +
            ...
            "where sc.status_contract_id=3 and tc.type_contract_id=1 and (c.start_date>= :startDate  and c.end_date<= :endDate)", nativeQuery = true)
    ...
    @Query(value = "select *" +
            ...
            "join type_contract tc on tc.type_contract_id=c.type_contract_id " +
            "join status_contract sc on sc.status_contract_id=c.status_contract_id " +
            "where tc.type_contract_id=2 " +
            "and sc.status_contract_id=3 " +
            "and c.liquidation_date is not null " +
            "and (c.start_date>= :startDate  and c.end_date<= :endDate)", nativeQuery = true)
